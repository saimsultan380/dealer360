...
export function SaleForm() {
  ...
  const onSubmit = async (data: SaleFormValues) => {
    try {
      ...
      let buyerAvatarUrl: string | undefined;
      if (buyerImages.photo) {
        const supabase = createClient();
        const {
          data: { user },
        } = await supabase.auth.getUser();
        if (!user) throw new Error("Unauthorized");
        const fileExt = buyerImages.photo.name.split(".").pop() || "jpg";
        const fileName = `buyer-avatar-${Math.random()
          .toString(36)
          .substring(2)}-${Date.now()}.${fileExt}`;
        const filePath = `client-avatars/${fileName}`;
        const { error: uploadError } = await supabase.storage
          .from("vehicles")
          .upload(filePath, buyerImages.photo, { upsert: false });
        if (uploadError) throw new Error("Failed to upload buyer photo");
        const {
          data: { publicUrl },
        } = supabase.storage.from("vehicles").getPublicUrl(filePath);
        buyerAvatarUrl = publicUrl;
      }
      ...
      // Note: CNIC docs are still not uploaded here (only avatar photo is synced)
  ...
  const remainingAmount =
    downPayment !== undefined
      ? Math.max(0, (salePrice || 0) - (downPayment || 0))
      : 0;
  const profit = selectedVehicle
    ? (salePrice || selectedVehicle.selling_price || 0) -
          ...
          <CardHeader className="-mt-4 sm:-mt-6 pt-4 sm:pt-6 bg-gradient-to-r from-primary/10 via-primary/5 to-transparent border-b">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <CardDescription>
                  Choose a vehicle from your available inventory
                </CardDescription>
              </div>
            </div>
          </CardHeader>
            ...
            <CardHeader className="-mt-4 sm:-mt-6 pt-4 sm:pt-6 bg-gradient-to-r from-blue-500/10 via-blue-500/5 to-transparent border-b">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <CardDescription>
                    Select an existing client or add a new seller who originally
                    sold this vehicle
                  </CardDescription>
                </div>
              </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    ...
                    {selectedVehicle.purchase_price > 0 && (
                      <p className="text-xs text-muted-foreground">
                        {(
                          (profit / selectedVehicle.purchase_price) *
                          100
                        ).toFixed(1)}
                        % margin
                      </p>
      ...
      {currentStep === 5 && (
        <Card className="overflow-hidden">
          ...
          <CardContent className="pt-6 space-y-4">
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div className="rounded-lg border p-3">
                ...
                <div className="font-semibold">
                  PKR {(watch("sale_price") || 0).toLocaleString()}
                </div>
              </div>
              <div className="rounded-lg border p-3">
                ...
                <div className="font-semibold">
                  PKR {(watch("down_payment") || 0).toLocaleString()}
                </div>
              </div>
              <div className="rounded-lg border p-3">
                <div className="text-xs text-muted-foreground">Commission</div>
                <div className="font-semibold">
                  PKR {Number(watch("commission_amount") || 0).toLocaleString()}
                </div>
              </div>
              <div className="rounded-lg border p-3">
            ...
            <div className="text-xs text-muted-foreground">
              Buyer documents selected: {buyerImages.photo ? "Photo " : ""}
              {buyerImages.cnicFront ? "CNIC Front " : ""}
              {buyerImages.cnicBack ? "CNIC Back" : ""}
            </div>
          </CardContent>
        </Card>
      )}
