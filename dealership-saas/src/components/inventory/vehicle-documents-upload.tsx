...
export function VehicleDocumentsUpload({ onDocumentIdsChange, className }: VehicleDocumentsUploadProps) {
    const [uploading, setUploading] = useState(false);
    const [items, setItems] = useState<{ id: string; url: string; type: string; name: string }[]>([]);
    ...
    const uploadFiles = useCallback(
        async (files: FileList | File[], docType: string = 'registration') => {
            ...
            const uploaded: { id: string; url: string; type: string; name: string }[] = [];

            await Promise.all(
                fileArr.map(async (file) => {
                    const fileExt = file.name.split('.').pop() || 'jpg';
                    const fileName = `vehicle-doc-${docType}-${Math.random().toString(36).slice(2)}-${Date.now()}.${fileExt}`;
            ...
            setItems((prev) => {
                ...
                onDocumentIdsChange(next.map((x) => x.id));
                return next;
            });
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">
                        Upload registration, insurance, invoices, and other vehicle documents
                    </p>
                </div>
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <p className="text-sm font-medium">
                                        {DOCUMENT_TYPES.find((t) => t.value === item.type)?.label || 'Document'}
                                    </p>
