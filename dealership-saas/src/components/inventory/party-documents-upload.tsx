...
export function PartyDocumentsUpload({ party, onDocumentIdsChange, className }: PartyDocumentsUploadProps) {
    ...
    const title = useMemo(() => (party === 'buyer' ? 'Buyer Images' : 'Seller Images'), [party]);
        ...
        async (files: FileList | File[]) => {
            ...
            await Promise.all(
                fileArr.map(async (file) => {
                    ...
                    const fileName = `${party}-${Math.random().toString(36).slice(2)}-${Date.now()}.${fileExt}`;
                    ...
                    uploaded.push({ id: recordId, url: publicUrl || URL.createObjectURL(file) });
                })
            );
        ...
        <div className={cn('space-y-3', className)}>
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">
                        Capture or upload photos (CNIC, buyer/seller pictures, receipts, etc.)
                    </p>
                </div>
            ...
            >
                <DialogContent className="max-w-2xl">
                    <DialogHeader>
                        <DialogTitle>{party === 'buyer' ? 'Capture Buyer Image' : 'Capture Seller Image'}</DialogTitle>
                        <DialogDescription>
            ...
            {items.length > 0 ? (
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                    {items.map((item, index) => (
                        <div key={`${item.id}-${item.url}`} className="relative aspect-video group rounded-md overflow-hidden bg-muted">
                            <Image src={item.url} alt={`${party} document`} fill className="object-cover" />
                            <button
                                type="button"
