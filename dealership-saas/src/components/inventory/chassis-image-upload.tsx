...
export function ChassisImageUpload({ onImageUploaded, className }: ChassisImageUploadProps) {
    ...
    const uploadFile = useCallback(
        async (file: File) => {
            ...
            const fileName = `chassis-${Math.random().toString(36).slice(2)}-${Date.now()}.${fileExt}`;
        ...
        <div className={cn('space-y-3', className)}>
            ...
            {image ? (
                <div className="relative aspect-video w-full max-w-md rounded-md overflow-hidden bg-muted border group">
                    <Image src={image.url} alt="Chassis number" fill className="object-cover" />
                    <button
                        type="button"
                        onClick={removeImage}
