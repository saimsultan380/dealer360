    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
          ...
          {isMock && (
            <p className="text-sm text-amber-600 dark:text-amber-400 mt-1">
              ...
              {deal.id.startsWith("mock")
                ? "Configure Supabase for real data"
        ...
        <Link
          href={
            deal.status === "pending"
              ? "/dashboard/deals/pending"
              : "/dashboard/sales"
          }
        >
          <Button variant="outline">
            {deal.status === "pending"
              ? "Back to Pending Deals"
              : "Back to Sales"}
          </Button>
        </Link>
      </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="font-medium">
                PKR {parseFloat(deal.down_payment).toLocaleString()}
              </span>
            </div>
            {(deal.status === "completed" || deal.status === "sold") && (
              <div className="flex items-center justify-between">
                <span className="text-sm text-muted-foreground">
                  Paid Amount
                </span>
                <span className="font-medium text-green-600 dark:text-green-400">
                  PKR{" "}
                  {(
                    parseFloat(deal.sale_price) - parseFloat(deal.down_payment)
                  ).toLocaleString()}
                </span>
              </div>
            )}
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center gap-4">
              <Avatar className="h-14 w-14">
                <AvatarImage
                  src={deal.customer_avatar_url ?? undefined}
                  alt={deal.customer_name || "Customer"}
                />
                <AvatarFallback className="bg-primary/10 text-primary text-lg">
                  {deal.customer_name?.charAt(0)?.toUpperCase() || "C"}
                </AvatarFallback>
              </Avatar>
              <div>
            ...
            {primaryImage && (
              <div className="mt-4">
                <img
                  ...
                  className="w-full h-48 object-contain bg-black/5 dark:bg-white/5 rounded-lg"
                />
              </div>
            )}
...
export default async function DealDetailPage({
  ...
  try {
    ...
    // Only calculate remaining amount if deal is not completed/sold
    if (deal.status !== "completed" && deal.status !== "sold") {
      remainingAmount =
        parseFloat(deal.sale_price) - parseFloat(deal.down_payment);
    }
    ...
    // Final safety check: if deal is completed/sold, remaining amount must be 0
    if (deal.status === "completed" || deal.status === "sold") {
      remainingAmount = 0;
    }

    // Fetch client avatar by customer_phone so customer image shows when they exist in clients
    let customer_avatar_url: string | undefined;
    if (deal.customer_phone && profile?.organization_id) {
      const { data: client } = await supabase
        .from("clients")
        .select("avatar_url")
        .eq("organization_id", profile.organization_id)
        .eq("phone", deal.customer_phone)
        .maybeSingle();
      customer_avatar_url = getClientAvatarUrl(client?.avatar_url) ?? undefined;
    }
    const dealWithAvatar = { ...deal, customer_avatar_url };

    // Real deal from Supabase
    const vehicle = deal.vehicles;
    const ui = renderDealDetail(
      dealWithAvatar,
      vehicle,
      remainingAmount,
      false
        ...
        <div className="space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Trade-in vehicle was added to your inventory.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Link
                href={`/dashboard/inventory/${exchangeMeta.tradeInVehicleId}`}
              >
