...
export default function InvestorDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    if (error || !investor) {
        return (
            <div className="space-y-8">
                <div>
                    <Button variant="ghost" onClick={() => router.push('/dashboard/investors')}>
                ...
                <Card>
                    <CardContent className="pt-6">
                        <p className="text-destructive">Error: {error || 'Investor not found'}</p>
                    </CardContent>
                </Card>
            </div>
        );
        ...
        <div className="space-y-8">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-muted-foreground">Investor details and transactions</p>
                    </div>
                </div>
                <div className="flex items-center gap-2">
                    <Button
                        variant="outline"
                        onClick={() => router.push(`/dashboard/investors/${investor.id}/edit`)}
                    >
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                        ...
                        <div className="flex items-start gap-3">
                            ...
                            <div className="min-w-0">
                                ...
                                <p className="text-sm text-muted-foreground break-words">{investor.phone}</p>
                            </div>
                        </div>
                        {investor.email ? (
                            <div className="flex items-start gap-3">
                                ...
                                <div className="min-w-0">
                                    ...
                                    <p className="text-sm text-muted-foreground break-words">{investor.email}</p>
                                </div>
                            </div>
                        ) : (
                            <div></div>
                        ...
                        {investor.cnic ? (
                            <div className="flex items-start gap-3">
                                ...
                                <div className="min-w-0">
                                    ...
                                    <p className="text-sm text-muted-foreground break-words">{investor.cnic}</p>
                                </div>
                            </div>
                        ) : (
                            <div></div>
                        ...
                        {investor.address && (
                            <div className="flex items-start gap-3 md:col-span-2">
                                ...
                                <div className="min-w-0 flex-1">
                                    ...
                                    <p className="text-sm text-muted-foreground break-words">{investor.address}</p>
                                </div>
                            </div>
                        )}
                        ...
                        {investor.notes && (
                            <div className="flex items-start gap-3 md:col-span-2">
                                <div className="min-w-0 flex-1">
                                    ...
                                    <p className="text-sm text-muted-foreground break-words whitespace-pre-wrap">{investor.notes}</p>
                                </div>
                            </div>
                        )}
                    </div>
                        ...
                        <Table>
                            ...
                            <TableBody>
                                {transactions.map((transaction) => {
                                    ...
                                    const isYesterday = transactionDate.toDateString() === new Date(Date.now() - 86400000).toDateString();
                                    
                                    let dateDisplay = transactionDate.toLocaleDateString('en-US', {
                                        year: 'numeric',
                                        month: 'short',
                                        day: 'numeric'
                                    });
                                        ...
                                        <TableCell className="text-right font-semibold">
                                            {transaction.transaction_type === 'investment' ? (
                                                <span className="text-green-600">
                                                    +PKR {parseFloat(transaction.amount.toString()).toLocaleString()}
                                                </span>
                                            ) : (
            ...
            <Dialog open={transactionDialogOpen} onOpenChange={setTransactionDialogOpen}>
                <DialogContent className="max-w-2xl">
                    <DialogHeader>
                        <DialogTitle>
                            {transactionType === 'investment' ? 'Add Investment' : 'Add Withdrawal'}
                        </DialogTitle>
                        <DialogDescription>
