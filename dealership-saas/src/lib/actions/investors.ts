...
export async function getInvestors(): Promise<{ data: InvestorWithBalance[] | null; error: string | null }> {
    try {
...
export async function getInvestorById(id: string): Promise<{ data: InvestorWithBalance | null; error: string | null }> {
    try {
        const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL;
        const supabaseAnonKey = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY;
...
export async function createInvestor(data: InvestorFormData): Promise<{ data: Investor | null; error: string | null }> {
    try {
        const supabase = (await createClient()) as any;
        const { data: { user } } = await supabase.auth.getUser();
...
export async function deleteInvestor(id: string): Promise<{ success: boolean; error: string | null }> {
    try {
        const supabase = (await createClient()) as any;
        const { data: { user } } = await supabase.auth.getUser();
