...
export async function getVehicles({
    ...
    try {
        ...
        if (!isSupabaseConfigured) {
            ...
            const mockVehicles = [
                ...
                },
                {
                    ...
                    fuel_type: 'hybrid',
                    transmission: 'automatic',
                    description: 'Toyota Camry Hybrid - recently sold. Mock data for testing.',
                    added_by: 'mock-user-1',
                ...
                },
                {
                    ...
                    fuel_type: 'petrol',
                    transmission: 'automatic',
                    description: 'Honda City Aspire currently in service. Mock data for testing.',
                    added_by: 'mock-user-1',
            ...
            if (query) {
                ...
                filteredVehicles = filteredVehicles.filter(v => 
                    ...
                    (v.registration_number && v.registration_number.toLowerCase().includes(searchLower))
                );
            }
        ...
        if (tableCheckError) {
            ...
            if (errorMsg.includes('relation') && errorMsg.includes('does not exist')) {
                return {
                    ...
                    error: 'Vehicles table does not exist. Please run the SUPABASE_QUICK_SETUP.sql script in your Supabase SQL Editor.',
                };
            }
            ...
            if (errorMsg.includes('permission denied') || errorMsg.includes('policy')) {
                return {
                    data: [],
                    metadata: { total: 0, page, limit, totalPages: 0 },
                    error: 'Permission denied. Please check Row Level Security policies. Make sure you have run the RLS policies from SUPABASE_QUICK_SETUP.sql',
                };
            }
        }
        ...
        if (query) {
            ...
            dbQuery = dbQuery.or(`make.ilike.${escapedPattern}, model.ilike.${escapedPattern}, registration_number.ilike.${escapedPattern}`);
        }
        ...
        if (error) {
            ...
            if (errorMessage.includes('relation') && errorMessage.includes('does not exist')) {
                userFriendlyMessage = 'Vehicles table does not exist. Please run the database setup SQL script.';
            } else if (errorMessage.includes('permission denied') || errorMessage.includes('RLS')) {
                userFriendlyMessage = 'Permission denied. Please check Row Level Security policies.';
            } else if (errorMessage.includes('JWT')) {
                userFriendlyMessage = 'Authentication error. Please check your Supabase configuration.';
            }
...
export async function createVehicle(
    ...
    if (!isSupabaseConfigured) {
        ...
        console.warn('⚠️ Supabase not configured. Create vehicle action is simulated.');
    ...
    if (imageUrls.length > 0) {
        ...
        const { error: imagesError } = await (supabase as any).from('vehicle_images').insert(rows);
    ...
    const allDocIds = [...buyerDocumentIds, ...sellerDocumentIds, chassisImageId, ...vehicleDocumentIds].filter(Boolean);
        ...
        try {
            await (supabase as any)
                ...
                .insert({
                    ...
                    file_url: `data:application/json;base64,${Buffer.from(JSON.stringify(metadata)).toString('base64')}`,
                    uploaded_by: user.id,
                });
        } catch (e) {
        ...
        if (
            ...
            try {
                ...
                if (buyerResult.data) {
                    ...
                    if (party.paymentDetails && (party.paymentDetails.downPayment || data.selling_price)) {
                        const totalAmount = parseFloat(data.selling_price?.toString() || '0') || parseFloat(data.purchase_price?.toString() || '0');
                        ...
                        if (totalAmount > 0) {
                            try {
                                await createClientTransaction({
                                    ...
                                    transaction_date: party.paymentDetails.paymentDate || new Date().toISOString(),
                                    status: 'completed',
    ...
    try {
        if (party?.sellerDetails?.party_type === 'investor' && party.sellerDetails.investor_id && data.purchase_price) {
            ...
            try {
                ...
                console.error('Error creating investor withdrawal for vehicle purchase:', e);
            }
            ...
            try {
                await (supabase as any).from('cash_transactions').insert({
                    ...
                    currency: 'PKR',
                    payment_method: 'cash',
                    description: `Vehicle purchase from investor (${party.sellerDetails.name || 'Investor'})`,
                ...
                console.error('Error creating cash transaction for investor vehicle purchase:', e);
            }
        }

        if (party?.buyerDetails?.party_type === 'investor' && party.buyerDetails.investor_id && data.selling_price) {
            // Vehicle sold to investor (record money coming from investor) - treat as investment
            const amount = party.paymentDetails?.downPayment ?? parseFloat(data.selling_price.toString());
            try {
                await (supabase as any).from('investment_transactions').insert({
                    organization_id: profile.organization_id,
                ...
                console.error('Error creating investor investment for vehicle sale:', e);
            }
            ...
            try {
                await (supabase as any).from('cash_transactions').insert({
                    ...
                    currency: 'PKR',
                    payment_method: party.paymentDetails?.method || 'cash',
                    description: `Vehicle sale to investor (${party.buyerDetails.name || 'Investor'})`,
                    reference_number: `VEH-${vehicle.id}`,
                    transaction_date: (party.paymentDetails?.paymentDate || new Date().toISOString()).slice(0, 10),
                    related_entity_type: 'investor',
                ...
                console.error('Error creating cash transaction for investor vehicle sale:', e);
            }
        }
    } catch (e) {
...
export async function updateVehicle(
    ...
    await (supabase as any).from('vehicle_images').delete().eq('vehicle_id', vehicleId).eq('organization_id', profile.organization_id);
    if (imageUrls.length > 0) {
        const rows = imageUrls
            .filter(Boolean)
            .map((url, i) => ({
        ...
        const { error: imgErr } = await (supabase as any).from('vehicle_images').insert(rows);
    ...
    const metadataUrl = `data:application/json;base64,${Buffer.from(JSON.stringify(metadata)).toString('base64')}`;
    ...
    if (existingMeta?.id) {
        await (supabase as any).from('documents').update({ file_url: metadataUrl } as any).eq('id', existingMeta.id);
    } else {
