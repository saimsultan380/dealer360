...
export async function getSales(filters?: {
  ...
  try {
    ...
    let query = supabase
      .from("deals")
      .select(
        "*, vehicles(make, model, year, variant, registration_number, vehicle_images(id, url, is_primary))",
        { count: "exact" }
      )
      .eq("organization_id", profile.organization_id)
    ...
    if (filters?.search) {
      ...
      query = query.or(
        `customer_name.ilike.${searchPattern}, customer_phone.ilike.${searchPattern}, customer_cnic.ilike.${searchPattern}`
      );
    }
    ...
    let enrichedSales = (data || []) as any[];

    // Attach client avatar_url to each sale by matching on customer_phone
    try {
      ...
      if (phones.length > 0) {
        ...
        if (!clientsError && clients) {
          const avatarByPhone = new Map<string, string | null>();
          for (const c of clients as any[]) {
            if (c.phone) {
              avatarByPhone.set(c.phone, c.avatar_url || null);
            }
          }

          enrichedSales = enrichedSales.map((sale) => ({
            ...sale,
            client_avatar_url: avatarByPhone.get(sale.customer_phone) ?? null,
          }));
        } else if (clientsError) {
          console.error(
            "Error fetching client avatars for sales:",
            clientsError
          );
        }
      }
    } catch (avatarErr) {
      console.error("Error enriching sales with client avatars:", avatarErr);
    }
...
export async function getSaleById(id: string) {
  try {
    ...
    const { data: sale, error } = await supabase
      .from("deals")
      .select(
        "*, vehicles(*, vehicle_images(id, url, is_primary, display_order)), profiles!deals_salesperson_id_fkey(full_name)"
      )
      .eq("id", id)
      .eq("organization_id", profile.organization_id)
      .single();
    ...
    // Attach client avatar by matching phone to clients table
    try {
      if (sale?.customer_phone) {
        ...
        if (!clientError && client) {
          return {
            data: { ...sale, client_avatar_url: client.avatar_url || null },
            error: null,
          };
        }
        if (clientError) {
          console.error(
            "Error fetching client avatar for sale detail:",
            clientError
          );
        }
      }
      ...
      console.error(
        "Error enriching sale detail with client avatar:",
        avatarErr
      );
    }
...
export async function createSale(data: SaleFormData) {
  try {
    ...
    if (
      ...
      console.warn(
        "⚠️ Supabase not configured. Create sale action is simulated."
      );
    ...
    const remainingAmount =
      data.remaining_amount || data.sale_price - data.down_payment;
    ...
    if (data.buyer_party_type === "investor" && data.buyer_investor_id) {
      ...
      try {
        const invAmount =
          data.down_payment > 0 ? data.down_payment : data.sale_price;
        ...
        console.error(
          "Error creating investment transaction for investor buyer:",
          e
        );
      }
      ...
      try {
        ...
        await (supabase as any).from("cash_transactions").insert({
          ...
          transaction_date: (data.deal_date || new Date().toISOString()).slice(
            0,
            10
          ),
      ...
      if (data.buyer_avatar_url) {
        try {
          await (supabase as any)
            .from("investors")
            .update({ avatar_url: data.buyer_avatar_url })
            .eq("id", data.buyer_investor_id)
            .eq("organization_id", profile.organization_id);
        } catch (e) {
          console.error("Error updating investor avatar for buyer:", e);
        }
      }
      ...
      try {
        ...
        if (!clientId) {
          ...
          const { data: newClient, error: clientError } = await supabase
            ...
            .insert({
              ...
              avatar_url: data.buyer_avatar_url || null,
              status: "active",
            })
            .select()
            .single();
        ...
        if (clientId) {
          ...
          if (transactionError) {
            console.error(
              "Error creating client transaction:",
              transactionError
            );
...
export async function updateSale(id: string, data: UpdateSaleData) {
  try {
    ...
    if (
      ...
      console.warn(
        "⚠️ Supabase not configured. Update sale action is simulated."
      );
      revalidatePath("/dashboard/sales");
      return { success: true };
    }
