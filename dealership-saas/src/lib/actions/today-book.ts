...
export async function getTodayActivities(): Promise<{
  ...
  try {
    ...
    const { data: cashIns, error: cashInsError } = await supabase
      ...
      .select(
        "id, amount, status, payment_method, created_at, transaction_type"
      )
      .eq("organization_id", profile.organization_id)
    ...
    if (cashInsError) {
      console.error(
        "Error fetching cash transactions for Today Book:",
        cashInsError
      );
    }
    ...
    const { data: activityLogs, error: logsError } = await supabase
      .from("activity_logs")
      .select(
        "id, action, entity_type, details, created_at, profiles(full_name)"
      )
      .eq("organization_id", profile.organization_id)
      .gte("created_at", todayStart)
      .lt("created_at", todayEnd)
    ...
    const completedDealsToday = (deals || []).filter(
      (d: any) => String(d.status || "").toLowerCase() === "completed"
    );
    const totalRevenue =
      completedDealsToday.reduce(
        (sum: number, deal: any) => sum + Number(deal.sale_price || 0),
        0
      ) || 0;
    const totalPayments =
      (cashIns || []).filter(
    ...
    (deals || [])
      .filter((d: any) => String(d.status || "").toLowerCase() === "completed")
      .forEach((d: any) => {
        const key = (d.payment_method || "unknown").toString().toLowerCase();
        revenueByPaymentMethodMap[key] =
          (revenueByPaymentMethodMap[key] || 0) + Number(d.sale_price || 0);
      });
    ...
    (weekDeals || [])
      ...
      .forEach((d: any) => {
        ...
        revenueByDate[dateKey] =
          (revenueByDate[dateKey] || 0) + Number(d.sale_price || 0);
      });
    ...
    (deals || []).forEach((deal: any) => {
      ...
      activities.push({
        ...
        description: `${vehicle?.make || ""} ${vehicle?.model || ""} ${
          vehicle?.year || ""
        } - PKR ${parseFloat(deal.sale_price || "0").toLocaleString()}`,
    ...
    (cashIns || []).forEach((payment: any) => {
      activities.push({
        ...
        type: "payment",
        title: "Payment Received",
        description: `${payment.payment_method || "Payment"} - PKR ${Number(
          payment.amount || 0
        ).toLocaleString()}`,
    ...
    (activityLogs || []).forEach((log: any) => {
      ...
      const alreadyAdded = activities.some(
        (a) =>
          a.metadata?.entity_id === log.id ||
          (log.entity_type === "deal" &&
            activities.some(
              (a) => a.type === "deal" && a.id === log.entity_id
            )) ||
          (log.entity_type === "vehicle" &&
            activities.some(
              (a) => a.type === "vehicle" && a.id === log.entity_id
            )) ||
          (log.entity_type === "lead" &&
            activities.some((a) => a.type === "lead" && a.id === log.entity_id))
      );
