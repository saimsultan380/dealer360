...
export interface ClientWithStats extends Client {
  total_dues: number;
  total_spent: number;
  /** Number of purchase-type transactions (client bought from dealership) */
  total_purchases: number;
  /** Number of sale-type transactions (dealership bought from client) */
  total_sales: number;
  /** Total number of deals (including exchange deals) linked to this client */
  total_deals: number;
...
export async function getClients(): Promise<{
  ...
  try {
    ...
    // Fetch all deals once (for dues + total_deals, including exchange deals)
    const { data: allDeals, error: dealsError } = await supabase
      .from("deals")
      .select("customer_phone, sale_price, down_payment, status")
      .eq("organization_id", profile.organization_id);

    if (dealsError) {
      console.error("Error fetching deals for clients:", dealsError);
    }

    const duesByPhone = new Map<string, number>();
    const dealsCountByPhone = new Map<string, number>();

    (allDeals || []).forEach((d: any) => {
      const phone = (d.customer_phone || "").trim();
      if (!phone) return;

      // Count all deals for this phone (sales + exchange)
      dealsCountByPhone.set(phone, (dealsCountByPhone.get(phone) || 0) + 1);

      // Only non-completed deals contribute to pending dues
      if (d.status === "completed") return;
      const remaining =
        parseFloat(d.sale_price || "0") - parseFloat(d.down_payment || "0");
      if (remaining <= 0) return;
      duesByPhone.set(phone, (duesByPhone.get(phone) || 0) + remaining);
    });

    // Calculate stats for each client
    const clientsWithStats: ClientWithStats[] = await Promise.all(
      (clients || []).map(async (client: any) => {
        // Get all completed transactions for this client (for total_spent / purchases / last_transaction_date)
        const { data: transactions } = await supabase
          .from("client_transactions")
          .select(
            "transaction_type, amount, remaining_due, transaction_date, status"
          )
        ...
        (transactions || []).forEach((tx: any) => {
          ...
          if (
            ...
            tx.transaction_date > last_transaction_date
          ) {
        ...
        const total_dues =
          client.phone && duesByPhone.has(client.phone)
            ? duesByPhone.get(client.phone) || 0
            : 0;
        const total_deals =
          client.phone && dealsCountByPhone.has(client.phone)
            ? dealsCountByPhone.get(client.phone) || 0
            : 0;
...
export async function getClientById(
  id: string
): Promise<{ data: ClientWithStats | null; error: string | null }> {
  try {
    const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL;
    const supabaseAnonKey = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY;
    ...
    // Compute dues based on non-completed deals for this client's phone
    let total_dues = 0;
    if (client.phone) {
      const { data: pendingDeals } = await supabase
        .from("deals")
        .select("sale_price, down_payment, status")
        .eq("organization_id", profile.organization_id)
        .eq("customer_phone", client.phone)
        .neq("status", "completed");

      (pendingDeals || []).forEach((d: any) => {
        const remaining =
          parseFloat(d.sale_price || "0") - parseFloat(d.down_payment || "0");
        if (remaining > 0) {
...
export async function findOrCreateClient(
  ...
  try {
    ...
    if (existingClient) {
      ...
      if (details.name && details.name !== existingClient.name)
        updateData.name = details.name;
      if (details.email && details.email !== existingClient.email)
        updateData.email = details.email;
      if (details.cnic && details.cnic !== existingClient.cnic)
        updateData.cnic = details.cnic;
      if (details.address && details.address !== existingClient.address)
        updateData.address = details.address;
      if (
        details.avatar_url !== undefined &&
        details.avatar_url !== existingClient.avatar_url
      )
        updateData.avatar_url = details.avatar_url || null;
    ...
    return {
      data: null,
      error:
        err instanceof Error ? err.message : "Failed to find or create client.",
    };
  }
}
  ...
  try {
    ...
    if (
      ...
      const mockTransaction: ClientTransaction = {
        ...
        remaining_due:
          data.remaining_due ||
          (data.total_amount ? data.total_amount - (data.paid_amount || 0) : 0),
        notes: data.notes || null,
        created_by: null,
        created_at: new Date().toISOString(),
        updated_at: new Date().toISOString(),
